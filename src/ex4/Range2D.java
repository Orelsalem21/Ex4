...
public class Range2D {
    ...
    public Range2D(String rangeStr) {
        ...
        ex4.CellEntry start = new ex4.CellEntry(parts[0].trim());
        ex4.CellEntry end = new ex4.CellEntry(parts[1].trim());
     ...
     */
    public ArrayList<ex4.Index2D> getAllCells() {
        ArrayList<ex4.Index2D> list = new ArrayList<>();
        for (int col = colStart; col <= colEnd; col++) {
            for (int row = rowStart; row <= rowEnd; row++) {
                // Construct cell reference using Ex2Utils.ABC for column letter.
                String cellRef = ex4.Ex2Utils.ABC[col] + row;
                list.add(new ex4.CellEntry(cellRef));
            }
        }
        return list;
    }
